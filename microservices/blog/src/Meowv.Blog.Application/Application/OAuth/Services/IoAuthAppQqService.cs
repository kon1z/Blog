...
public class IoAuthAppQqService : IoAuthAppServiceBase<QQOptions, AccessTokenBase, QQUserInfo>
{
    ...
    public override async Task<UserDto> GetUserByOAuthAsync(string type, string code, string state)
    {
        ...
        return await UserAppService.CreateUserAsync(userInfo.Name, type, userInfo.Id, userInfo.Name, userInfo.Avatar,
            userInfo.Email);
    }
    ...
    public override async Task<QQUserInfo> GetUserInfoAsync(AccessTokenBase accessToken)
    {
        ...
        var openIdResponse =
            await client.GetStringAsync($"{Options.Value.OpenIdUrl}?access_token={accessToken.AccessToken}&fmt=json");
