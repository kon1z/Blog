...
public class IoAuthAppGiteeService : IoAuthAppServiceBase<GiteeOptions, GiteeAccessToken, UserInfoBase>
{
    ...
    public override async Task<UserDto> GetUserByOAuthAsync(string type, string code, string state)
    {
        ...
        return await UserAppService.CreateUserAsync(userInfo.Login, type, userInfo.Id, userInfo.Name, userInfo.Avatar,
            userInfo.Email);
    }
    ...
    public override async Task<UserInfoBase> GetUserInfoAsync(GiteeAccessToken accessToken)
    {
        ...
        var response =
            await client.GetStringAsync($"{Options.Value.UserInfoUrl}?access_token={accessToken.AccessToken}");
