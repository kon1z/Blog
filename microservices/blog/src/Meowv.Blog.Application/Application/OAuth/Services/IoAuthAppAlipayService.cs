...
public class IoAuthAppAlipayService : IoAuthAppServiceBase<AlipayOptions, AlipayAccessToken, AlipayUserInfo>
{
    ...
    public override async Task<UserDto> GetUserByOAuthAsync(string type, string code, string state)
    {
        var accessToken = await GetAccessTokenAsync(code, state);
        var userInfo = await GetUserInfoAsync(accessToken);

        return await UserAppService.CreateUserAsync(userInfo.UserInfoResponse.Name, type, userInfo.UserInfoResponse.Id,
            userInfo.UserInfoResponse.Name, userInfo.UserInfoResponse.Avatar, userInfo.UserInfoResponse.Email); 
    }
