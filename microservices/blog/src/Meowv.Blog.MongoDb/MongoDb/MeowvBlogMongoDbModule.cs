...
public class MeowvBlogMongoDbModule : AbpModule
{
    ...
    {
        context.Services.AddMongoDbContext<MeowvBlogMongoDbContext>(options => { options.AddDefaultRepositories(); });
        ...
        Configure<AbpAuditingOptions>(options => { options.IsEnabledForGetRequests = true; });
